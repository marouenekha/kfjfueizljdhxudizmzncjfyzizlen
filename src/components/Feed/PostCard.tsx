...
export function PostCard({ post, onPostUpdated, onPostDeleted }: PostCardProps) {
  ...
  return (
    <div className="bg-card border-b border-border">
      ...
      {post.shared_post_id && (
        <div className="flex items-center gap-1.5 px-4 pt-2.5 pb-0 text-xs text-muted-foreground">
          <Repeat2 className="w-3.5 h-3.5" />
          <span className="font-medium cursor-pointer hover:underline" onClick={() => post.user_id && navigate(`/profile?user=${post.user_id}`)}>
            {post.user_id === authUser?.id ? t('you') || 'You' : post.user_name}
          </span>
          <span>{t('sharedAPost') || 'shared a post'}</span>
        </div>
      )}

      {/* Sharer's comment (if any, for reposts) */}
      ...
      {/* For reposts: embedded original post card */}
      {post.shared_post_id ? (
        <div className="mx-4 my-2 border border-border rounded-lg overflow-hidden bg-muted/20">
          {/* Original author header */}
          <div className="flex items-center gap-2.5 px-3 py-2.5">
            <Avatar className="w-8 h-8 cursor-pointer"
              onClick={() => post.original_user_id && navigate(`/profile?user=${post.original_user_id}`)}>
              <AvatarImage src={post.original_user_avatar || undefined} />
              <AvatarFallback className="bg-primary/10 text-primary text-xs font-semibold">
                {post.original_user_name?.[0] || "U"}
              </AvatarFallback>
            </Avatar>
            <div>
              ...
              <Badge variant={post.post_type === "find" ? "default" : "secondary"} className="text-[10px] px-1.5 py-0 ml-2 rounded-full">
          ...
          {/* Original post content (from the original, not the sharer's comment) */}
          {post.images && post.images.length > 0 && (
            <div className="px-3 py-2">
              <p className="text-xs text-muted-foreground">{post.original_user_name}'s post</p>
            </div>
          )}
        </div>
      ) : (
