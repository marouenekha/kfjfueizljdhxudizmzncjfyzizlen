export default {
  // Navigation
  home: "الرئيسية",
  search: "البحث",
  post: "نشر",
  messages: "الرسائل",
  profile: "الملف الشخصي",
  settings: "الإعدادات",
  logout: "تسجيل الخروج",
  back: "رجوع",
  backToHome: "العودة للرئيسية",

  // Auth
  login: "تسجيل الدخول",
  signup: "إنشاء حساب",
  email: "البريد الإلكتروني",
  password: "كلمة المرور",
  confirmPassword: "تأكيد كلمة المرور",
  fullName: "الاسم الكامل",
  signInWith: "سجل الدخول باستخدام",
  orContinueWith: "أو تابع باستخدام",
  forgotPassword: "نسيت كلمة المرور؟",
  forgotPasswordTitle: "إعادة تعيين كلمة المرور",
  forgotPasswordDescription: "أدخل بريدك الإلكتروني وسنرسل لك رابطًا لإعادة تعيين كلمة المرور.",
  sendResetLink: "إرسال رابط إعادة التعيين",
  resetLinkSent: "تم إرسال الرابط!",
  resetLinkSentDescription: "تحقق من بريدك الإلكتروني للعثور على رابط إعادة التعيين.",
  resetPassword: "إعادة تعيين كلمة المرور",
  passwordResetSuccess: "تم إعادة تعيين كلمة المرور!",
  passwordResetSuccessDescription: "يمكنك الآن تسجيل الدخول بكلمة المرور الجديدة.",
  passwordResetError: "فشل إعادة تعيين كلمة المرور",
  invalidResetLink: "رابط غير صالح أو منتهي الصلاحية",
  invalidResetLinkDescription: "يرجى طلب رابط جديد لإعادة التعيين.",
  noAccount: "ليس لديك حساب؟ سجل الآن",
  hasAccount: "لديك حساب بالفعل؟ سجل الدخول",
  loginSuccess: "تم تسجيل الدخول بنجاح",
  signupSuccess: "تم إنشاء الحساب بنجاح",
  welcome: "مرحباً!",
  loading: "جاري التحميل...",
  termsAgreement: "بتسجيل الدخول، أنت توافق على شروط الاستخدام وسياسة الخصوصية.",
  emailRequired: "البريد الإلكتروني مطلوب",
  emailInvalid: "بريد إلكتروني غير صالح",
  passwordRequired: "كلمة المرور مطلوبة",
  passwordMinLength: "يجب أن تتكون كلمة المرور من 6 أحرف على الأقل",
  nameRequired: "الاسم مطلوب",
  passwordsDontMatch: "كلمات المرور غير متطابقة",
  loginFailed: "فشل تسجيل الدخول",
  brandDescription: "اعثر على أفضل مقدمي الخدمات المحليين أو اعرض خدماتك لمجتمعك.",
  services: "🔧 خدمات",
  reviews: "⭐ تقييمات",
  messagesShort: "💬 رسائل",

  // Landing Page
  launchingBadge: "✨ إطلاق في دبي وأبوظبي",
  heroTitle: "سوقك المحلي للخدمات",
  heroDescription: "تواصل مع مقدمي خدمات محليين موثوقين لكل شيء من إصلاحات المنزل إلى التصميم الرقمي. آمن ومتعدد اللغات.",
  getStarted: "ابدأ الآن",
  imAProvider: "أنا مقدم خدمة",
  providers: "مقدمو خدمات",
  jobsDone: "أعمال منجزة",
  avgRating: "متوسط التقييم",
  browseCategories: "تصفح فئات الخدمات",
  browseCategoriesDesc: "اعثر على المزود المثالي لأي خدمة تحتاجها",
  homeServices: "خدمات منزلية",
  digitalServices: "خدمات رقمية",
  events: "فعاليات",
  wellness: "صحة وعافية",
  business: "أعمال",
  providersCount: "مقدمو خدمات",
  whyChoose: "لماذا تختار ServiceHub؟",
  whyChooseDesc: "مصمم لاقتصاد الخدمات المحلية الحديث",
  connectProviders: "تواصل مع مقدمي الخدمات المحليين",
  connectProvidersDesc: "اعثر على مقدمي خدمات معتمدين في منطقتك لأي نوع من الخدمات.",
  securePayments: "مدفوعات آمنة",
  securePaymentsDesc: "نظام دفع مدمج مع حماية الضمان للمعاملات الآمنة.",
  qualityGuaranteed: "جودة مضمونة",
  qualityGuaranteedDesc: "اقرأ التقييمات والمراجعات من عملاء حقيقيين لاتخاذ قرارات مدروسة.",
  multilingualSupport: "دعم متعدد اللغات",
  multilingualSupportDesc: "استخدم التطبيق بالعربية أو الإنجليزية أو الفرنسية مع دعم كامل للكتابة من اليمين لليسار.",
  readyToStart: "مستعد للبدء؟",
  readyToStartDesc: "انضم لآلاف العملاء ومقدمي الخدمات الراضين",
  findServices: "ابحث عن خدمات",
  becomeProvider: "كن مقدم خدمة",
  allRightsReserved: "جميع الحقوق محفوظة.",
  madeInUAE: "🇦🇪 صنع في الإمارات • متاح بالعربية، English، و Français",

  // Post Creation / Editing
  createPost: "إنشاء منشور",
  editPost: "تعديل المنشور",
  whatLookingFor: "ما الذي تبحث عنه؟",
  findAService: "ابحث عن خدمة",
  iNeedHelp: "أحتاج مساعدة",
  provideAService: "قدم خدمة",
  iOfferHelp: "أقدم المساعدة",
  addMedia: "إضافة وسائط (اختياري)",
  image: "صورة",
  video: "فيديو",
  carousel: "معرض صور",
  tapToAddImage: "انقر لإضافة صورة (1:1)",
  tapToAddVideo: "انقر لإضافة فيديو (9:16)",
  addImages: "إضافة صور",
  description: "الوصف",
  describeService: "صف الخدمة التي تبحث عنها أو تقدمها...",
  publishPost: "نشر المنشور",
  publishing: "جاري النشر...",
  saveChanges: "حفظ التغييرات",
  postCreated: "تم إنشاء المنشور!",
  postCreatedDesc: "منشورك الآن مباشر.",
  postUpdated: "تم تحديث المنشور!",
  postUpdatedDesc: "تم حفظ تغييراتك.",
  imageCount: "صورة (صور) — اسحب للمعاينة",
  cropImage: "قص الصورة",
  imageOf: "صورة {{current}} من {{total}}",

  // Feed
  feed: "الخلاصة",
  market: "السوق",
  discoverServices: "اكتشف الخدمات",
  noPostsYet: "لا توجد منشورات بعد",
  beFirstToPost: "كن أول من ينشر طلب خدمة أو عرض!",

  // Post Card
  find: "بحث",
  provide: "تقديم",
  more: "المزيد",
  contact: "تواصل",
  like: "إعجاب",
  likes: "إعجابات",
  viewAllComments: "عرض جميع {{count}} تعليقات",
  addComment: "أضف تعليقاً...",
  editPostMenu: "تعديل المنشور",
  deletePost: "حذف المنشور",
  deletePostTitle: "حذف المنشور",
  deletePostDesc: "هل أنت متأكد أنك تريد حذف هذا المنشور؟ لا يمكن التراجع عن هذا الإجراء.",
  cancel: "إلغاء",
  delete: "حذف",
  postDeleted: "تم حذف المنشور بنجاح",
  failedToDeletePost: "فشل حذف المنشور",

  // Share
  sharePost: "مشاركة المنشور",
  quickShare: "مشاركة سريعة",
  copyLink: "نسخ الرابط",
  shareExternal: "مشاركة خارجية",
  sendToUsers: "إرسال للمستخدمين",
  searchUsers: "البحث عن مستخدمين...",
  selected: "المحدد",
  noUsersFound: "لم يتم العثور على مستخدمين",
  sending: "جاري الإرسال...",
  sendTo: "إرسال إلى {{count}} {{people}}",
  person: "شخص",
  people: "أشخاص",
  postShared: "تمت مشاركة المنشور مع {{count}} {{people}}!",
  postLinkCopied: "تم نسخ رابط المنشور!",
  failedToCopyLink: "فشل نسخ الرابط",
  failedToSharePost: "فشل مشاركة المنشور",
  selectAtLeastOne: "يرجى اختيار شخص واحد على الأقل للمشاركة معه",
  serviceRequest: "طلب خدمة",
  serviceOffer: "عرض خدمة",

  // Profile
  editProfile: "تعديل الملف الشخصي",
  follow: "متابعة",
  following: "متابَع",
  rate: "تقييم",
  message: "رسالة",
  whatsapp: "واتساب",
  whatsappNumber: "رقم واتساب",
  whatsappHint: "أدرج رمز الدولة (مثال: ‎+213). يُستخدم ليتمكن الآخرون من التواصل معك عبر واتساب.",
  addWhatsappPrompt: "أضف رقم واتساب حتى يتمكن المشترون من التواصل معك.",
  addWhatsappCta: "أضف الآن",
  provider: "مقدم خدمة",
  joined: "انضم",
  followers: "متابعون",
  jobsCompleted: "أعمال مكتملة",
  rating: "التقييم",
  noRatings: "لا توجد تقييمات",
  posts: "المنشورات",
  portfolio: "الأعمال",
  noPostExists: "لا توجد منشورات",
  noPortfolioItems: "لا توجد أعمال بعد",
  noReviews: "لا توجد تقييمات",
  noReviewsOwn: "أكمل أول عمل لك للحصول على تقييمات!",
  noReviewsOther: "لم يتلق هذا المستخدم أي تقييمات بعد.",
  loadingProfile: "جاري تحميل الملف الشخصي...",
  pleaseSignIn: "يرجى تسجيل الدخول",
  needLoginProfile: "تحتاج لتسجيل الدخول لعرض هذا الملف الشخصي.",
  signIn: "تسجيل الدخول",

  // Profile Edit Dialog
  editProfileTitle: "تعديل الملف الشخصي",
  uploadCropPicture: "انقر لتحميل وقص صورة ملفك الشخصي",
  name: "الاسم",
  enterYourName: "أدخل اسمك",
  bio: "نبذة",
  tellAboutYourself: "أخبرنا عن نفسك",
  location: "الموقع",
  enterYourLocation: "أدخل موقعك",
  phone: "الهاتف",
  yourPhoneNumber: "رقم هاتفك",
  imServiceProvider: "أنا مقدم خدمة",
  serviceTypes: "أنواع الخدمات",
  saving: "جاري الحفظ...",
  profileUpdated: "تم تحديث الملف الشخصي!",
  profileUpdatedDesc: "تم تحديث ملفك الشخصي بنجاح.",
  updateFailed: "فشل التحديث",

  // Rating Modal
  rateUser: "تقييم {{name}}",
  clickToRate: "انقر للتقييم",
  stars: "{{count}} نجمة (نجوم)",
  commentOptional: "تعليق (اختياري)",
  shareExperience: "شارك تجربتك...",
  submitting: "جاري الإرسال...",
  submitRating: "إرسال التقييم",
  ratingSubmitted: "تم إرسال التقييم!",
  ratingSubmittedDesc: "قيّمت {{name}} بـ {{rating}} نجمة (نجوم).",
  ratingFailed: "فشل التقييم",

  // Settings
  language: "اللغة",
  selectLanguage: "اختر اللغة",
  changeName: "تغيير الاسم",
  updateName: "تحديث الاسم",
  changeEmail: "تغيير البريد الإلكتروني",
  emailAddress: "عنوان البريد الإلكتروني",
  enterYourEmail: "أدخل بريدك الإلكتروني",
  updateEmail: "تحديث البريد الإلكتروني",
  changePassword: "تغيير كلمة المرور",
  currentPassword: "كلمة المرور الحالية",
  enterCurrentPassword: "أدخل كلمة المرور الحالية",
  newPassword: "كلمة المرور الجديدة",
  enterNewPassword: "أدخل كلمة المرور الجديدة",
  confirmNewPassword: "تأكيد كلمة المرور الجديدة",
  deleteAccount: "حذف الحساب",
  deleteAccountWarning: "لا يمكن التراجع عن هذا الإجراء. سيتم حذف حسابك وجميع البيانات المرتبطة نهائياً.",
  areYouSure: "هل أنت متأكد تماماً؟",
  deleteAccountConfirm: "لا يمكن التراجع عن هذا الإجراء. سيتم حذف حسابك نهائياً وإزالة جميع بياناتك من خوادمنا.",
  nameUpdatedSuccessfully: "تم تحديث الاسم بنجاح",
  failedToUpdateName: "فشل تحديث الاسم",
  emailUpdateSent: "تم إرسال تحديث البريد الإلكتروني",
  checkEmailConfirm: "تحقق من بريدك الإلكتروني لتأكيد التغيير",
  failedToUpdateEmail: "فشل تحديث البريد الإلكتروني",
  passwordChangedSuccessfully: "تم تغيير كلمة المرور بنجاح",
  failedToChangePassword: "فشل تغيير كلمة المرور",
  accountDeletedSuccessfully: "تم حذف الحساب بنجاح",
  failedToDeleteAccount: "فشل حذف الحساب",

  // Messages
  startJob: "بدء العمل",
  jobInProgress: "العمل قيد التنفيذ",
  endJob: "إنهاء العمل",
  leaveReview: "اترك تقييماً",
  comment: "تعليق (اختياري)",
  submitReview: "إرسال التقييم",
  skip: "تخطي",
  typeMessage: "اكتب رسالة...",
  searchConversations: "البحث في المحادثات...",
  online: "متصل",
  noMessagesYet: "لا توجد رسائل بعد",
  startConversation: "ابدأ المحادثة!",
  noConversationsYet: "لا توجد محادثات بعد",
  startChatting: "ابدأ الدردشة مع مقدمي الخدمات!",
  yesterday: "أمس",

  // Search
  searchProfiles: "البحث عن ملفات شخصية",
  searchPlaceholder: "البحث عن ملفات شخصية أو خدمات أو مواقع...",
  selectRegion: "اختر المنطقة",
  clear: "مسح",
  profilesFound: "تم العثور على {{count}} ملف شخصي",
  sortByRating: "ترتيب حسب التقييم",
  viewProfile: "عرض الملف الشخصي",
  noProfilesFound: "لم يتم العثور على ملفات شخصية",
  tryAdjusting: "حاول تعديل البحث أو الفلاتر",

  // Notifications
  notifications: "الإشعارات",
  loadingNotifications: "جاري التحميل…",
  noNotificationsYet: "لا توجد إشعارات بعد",
  startedFollowingYou: "بدأ بمتابعتك",
  ratedYouStars: "قيّمك {{rating}} نجمة (نجوم)",

  // Provider Signup
  becomeAProvider: "كن مقدم خدمة",
  joinAsProvider: "انضم كمقدم خدمة",
  joinAsProviderDesc: "أنشئ ملفك كمقدم خدمة وابدأ بالتواصل مع العملاء",
  businessName: "اسم النشاط التجاري",
  yourBusinessName: "اسم نشاطك التجاري",
  phoneNumber: "رقم الهاتف",
  serviceCategory: "فئة الخدمة",
  selectCategory: "اختر الفئة",
  serviceLocation: "موقع الخدمة",
  yearsExperience: "سنوات الخبرة",
  yearsExperienceExample: "مثال: 5 سنوات",
  businessDescription: "وصف النشاط التجاري",
  businessDescriptionPlaceholder: "أخبرنا عن نشاطك التجاري وخدماتك...",
  uploadDocs: "تحميل مستندات العمل",
  chooseFiles: "اختر ملفات",
  docsOptional: "رخصة تجارية، شهادات تأمين (اختياري)",
  agreeTerms: "أوافق على شروط الخدمة وسياسة الخصوصية",
  agreeTermsDesc: "بالتسجيل، أنت توافق على الشروط وسياسة الخصوصية",
  termsRequired: "الشروط مطلوبة",
  pleaseAgreeTerms: "يرجى الموافقة على الشروط والأحكام",
  applicationSubmitted: "تم إرسال الطلب!",
  applicationSubmittedDesc: "سنراجع طلبك ونتواصل معك خلال 2-3 أيام عمل.",
  submitApplication: "إرسال الطلب",
  submittingApplication: "جاري إرسال الطلب...",
  failedToSubmit: "فشل إرسال الطلب. حاول مرة أخرى.",

  // Job Manager
  requestJob: "طلب عمل",
  requestJobFrom: "طلب عمل من {{name}}",
  jobTitle: "عنوان العمل",
  jobDescriptionOptional: "وصف العمل (اختياري)",
  sendRequest: "إرسال الطلب",
  jobRequested: "تم طلب العمل",
  jobRequestSent: "تم إرسال طلب العمل إلى {{name}}",
  jobUpdated: "تم تحديث العمل",
  showJobs: "عرض الأعمال",
  hideJobs: "إخفاء الأعمال",
  requestedByYou: "طُلب بواسطتك",
  requestedBy: "طُلب بواسطة {{name}}",
  accept: "قبول",
  refuse: "رفض",
  cancelJob: "إلغاء العمل",
  review: "تقييم",
  reviewUser: "تقييم {{name}}",
  reviewSubmitted: "تم إرسال التقييم",
  reviewSubmittedDesc: "قيّمت {{name}} بـ {{rating}} نجمة (نجوم)",
  errorFetchingJobs: "خطأ في جلب الأعمال",
  errorRequestingJob: "خطأ في طلب العمل",
  errorUpdatingJob: "خطأ في تحديث العمل",
  errorSubmittingReview: "خطأ في إرسال التقييم",

  // 404
  pageNotFound: "عذراً! الصفحة غير موجودة",
  returnToHome: "العودة للرئيسية",

  // Languages
  arabic: "العربية",
  french: "Français",
  english: "English",

  // Common
  error: "خطأ",
  postNotFound: "المنشور غير موجود",
  canOnlyEditOwn: "يمكنك فقط تعديل منشوراتك",

  // EditPostDialog
  pleaseAddContent: "يرجى إضافة محتوى لمنشورك",
  postUpdatedSuccess: "تم تحديث المنشور بنجاح!",
  failedToUpdatePost: "فشل تحديث المنشور",
  findService: "البحث عن خدمة",
  provideService: "تقديم خدمة",
  contentLabel: "المحتوى",
  describeServiceRequest: "صف طلب الخدمة أو عرضك...",
  currentImages: "الصور الحالية",
  imageEditingSoon: "تعديل الصور سيكون متاحاً قريباً",

  // Review Modal
  howWasExperience: "كيف كانت تجربتك مع {{name}}؟",

  // Job Manager status
  jobStatusRequested: "مطلوب",
  jobStatusAccepted: "مقبول",
  jobStatusInProgress: "قيد التنفيذ",
  jobStatusCompleted: "مكتمل",
  jobStatusCancelled: "ملغى",
  jobStatusRefused: "مرفوض",

  // Services list
  cleaning: "تنظيف",
  plumbing: "سباكة",
  electrical: "كهرباء",
  painting: "دهان",
  carpentry: "نجارة",
  gardening: "بستنة",
  moving: "نقل",
  tutoring: "تدريس",
  petCare: "رعاية حيوانات",
  photography: "تصوير",

  // Profile Roles
  profileRoleLabel: "نوع الملف الشخصي",
  role_provider: "مقدم خدمة",
  role_seller: "بائع",
  role_both: "كلاهما",
  profileRoleHint: "اختر ما تقدمه: خدمات أو منتجات أو كلاهما.",
  seller: "بائع",
  store: "متجر",
  noStoreItems: "لا توجد منتجات بعد",
  noStoreItemsDesc: "ستظهر المنتجات والعناصر المعروضة للبيع هنا.",
  addProduct: "إضافة منتج",
  editProduct: "تعديل المنتج",
  productTitle: "العنوان",
  productTitlePlaceholder: "اسم المنتج...",
  productDescription: "الوصف",
  productDescriptionPlaceholder: "صف منتجك...",
  productPrice: "السعر",
  productImages: "الصور",
  productAdded: "تمت إضافة المنتج!",
  productUpdated: "تم تحديث المنتج!",
  productDeleted: "تم حذف المنتج",
  searchProducts: "البحث في المنتجات…",
  noSearchResults: "لم يتم العثور على منتجات",

  // Creation screen
  create: "إنشاء",
  storeProduct: "منتج",
  createPortfolioItem: "عنصر معرض أعمال جديد",
  createStoreProduct: "منتج متجر جديد",
  projectImages: "صور المشروع",
  projectVideo: "فيديو المشروع",
  addVideoOptional: "إضافة فيديو (اختياري)",
  projectTitleLabel: "عنوان المشروع",
  projectTitlePlaceholder: "مثال: تجديد مطبخ...",
  portfolioDescPlaceholder: "صف مشروعك أو عملك المنجز...",
  tagsOptional: "وسوم (اختياري)",
  tagsPlaceholder: "مثال: تصميم، تجديد، هوية بصرية",
  publishPortfolio: "نشر المشروع",
  portfolioItemCreated: "تم إنشاء المشروع!",
  portfolioItemCreatedDesc: "تمت إضافة مشروعك إلى معرض أعمالك.",
  portfolioItemDeleted: "تم حذف المشروع",

  // Repost / Share
  repostToProfile: "إعادة النشر على الملف الشخصي",
  repostDesc: "شارك هذا المنشور على ملفك الشخصي",
  reposted: "تم المشاركة على ملفك الشخصي!",
  repostedDesc: "يظهر هذا المنشور الآن على ملفك الشخصي",
  repostedBy: "أعاد النشر",
  failedToRepost: "فشل في المشاركة",
  shareNow: "مشاركة الآن",
  shareWithText: "اكتب شيئاً وشارك",
  sharedAPost: "شارك منشوراً",
  sayAboutThis: "قل شيئاً عن هذا...",
  you: "أنت",
  addTextToShare: "أضف نصاً للمشاركة",

  // Shop / E-commerce
  product: "منتج",
  productNotFound: "المنتج غير موجود",
  noImage: "لا توجد صورة",
  soldBy: "بائع",
  addToCart: "أضف إلى السلة",
  addedToCart: "تمت الإضافة إلى السلة",
  buyNow: "اشترِ الآن",
  cart: "السلة",
  cartEmpty: "سلتك فارغة",
  cartEmptyDesc: "تصفح المتاجر وأضف منتجاتك المفضلة.",
  subtotal: "المجموع الفرعي",
  total: "المجموع",
  proceedToCheckout: "إتمام الطلب",
  checkout: "الدفع",
  orderSummary: "ملخص الطلب",
  deliveryInfo: "معلومات التوصيل",
  wilaya: "الولاية",
  commune: "البلدية",
  address: "العنوان",
  noteOptional: "ملاحظة (اختياري)",
  cashOnDelivery: "الدفع عند الاستلام",
  cashOnDeliveryDesc: "ادفع نقداً عند استلام طلبك",
  placeOrder: "تأكيد الطلب",
  orderPlaced: "تم تأكيد الطلب!",
  orderPlacedDesc: "سيستلم البائع طلبك قريباً.",
  missingFields: "يرجى ملء جميع الحقول المطلوبة",
  myOrders: "طلباتي",
  myPurchases: "مشترياتي",
  salesReceived: "المبيعات المستلمة",
  noOrders: "لا توجد طلبات بعد",
  noSales: "لا توجد مبيعات بعد",
  statusUpdated: "تم تحديث الحالة",
  status_pending: "قيد الانتظار",
  status_confirmed: "مؤكد",
  status_shipped: "تم الشحن",
  status_delivered: "تم التوصيل",
  status_cancelled: "ملغى",
};
